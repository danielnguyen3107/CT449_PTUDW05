...
exports.deleteAll = async (req, res, next) => {
  try {
    ...
    return res.send({ message: ` ${deletedCount} were deleted successfully` });
  } catch (error) {
    return next(new ApiError(500, `An Error occured while removing contacts `));
  }
};
...
exports.findAllFavorite = async (req, res, next) => {
  try {
    ...
    return next(
      new ApiError(500, `An Error occured while retrieving favourite contacts`)
    );
  }
};
